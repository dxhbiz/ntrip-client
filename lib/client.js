...
const { convertSource } = require('./source');
...
class NtripClient extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.isReady = false;
    /**
     * @member {Buffer} sourceBuf - source table buffer
     */
    this.sourceBuf = Buffer.alloc(0);
  }
   ...
   */
  async _loop() {
    if (this.interval <= 0 || this.mountpoint === '') {
      return;
    }
  ...
  _connect() {
    ...
    this.client.on('data', (data) => {
      if (this.mountpoint === '') {
        this.sourceBuf = Buffer.concat([this.sourceBuf, data]);
        return;
      }
  ...
  _onError(err) {
    ...
    if (this.sourceBuf.length > 0) {
      const tables = convertSource(this.sourceBuf);
      this._onData(tables);
      this.sourceBuf = Buffer.alloc(0);
    }
